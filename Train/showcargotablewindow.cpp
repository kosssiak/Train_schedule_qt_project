...
#include "ui_showcargotablewindow.h"

ShowCargoTableWindow::ShowCargoTableWindow(List<Cargo>* list) : ui(new Ui::ShowCargoTableWindow)
{
    ui->setupUi(this);
    ...
    for (Iterator it = list->begin(); it != list->end() + 1; ++it)
    {
        ...
        QString str;
        switch (list->operator[](i).getDestination())
        {
            case 1:
            {
                str = "Россия";
...
ShowCargoTableWindow::~ShowCargoTableWindow()
{
    delete ui;
}

void ShowCargoTableWindow::on_back_clicked()
{
    this->close();
    delete this;
}
